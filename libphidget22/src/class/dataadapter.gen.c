...
_bridgeInput(PhidgetChannelHandle phid, BridgePacket *bp) {
	...
	case BP_SETHANDSHAKEMODE:
		if (!supportedHandshakeMode(phid, (PhidgetDataAdapter_HandshakeMode)getBridgePacketInt32(bp,
		  0)))
			return (MOS_ERROR(bp->iop, EPHIDGET_INVALIDARG,
			  "Specified HandshakeMode is unsupported by this device."));
		res = DEVBRIDGEINPUT(phid, bp);
		if (res != EPHIDGET_OK) {
			break;
		}
...
_initAfterOpen(PhidgetChannelHandle phid) {
	...
	case PHIDCHUID_ADP_RS232_DATAADAPTER_100_USB:
		...
		ch->baudRate = 9600;
		ch->minBaudRate = 800;
		ch->maxBaudRate = 500000;
		ch->parity = PARITY_MODE_NONE;
		ch->stopBits = STOP_BITS_ONE;
		ch->dataBits = 8;
		ch->minDataBits = 7;
		ch->maxDataBits = 8;
		ch->handshakeMode = HANDSHAKE_MODE_NONE;
