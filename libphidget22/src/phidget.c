...
static PhidgetReturnCode transferpacket(mosiop_t iop, PhidgetDeviceHandle device, int transferType, int packetType, int index,
	const unsigned char *bufferIn, size_t *bufferInLen, PhidgetTransactionHandle trans, int timeout);
...
sendpacketWithTracking(mosiop_t iop, PhidgetDeviceHandle device, const unsigned char *buf, size_t length,
  PhidgetPacketTrackerHandle packetTracker, PhidgetTransactionHandle trans) {
	...
	if (trans) {
		...
		logverbose("%s Device: %p Packet: %p Sent", device->deviceInfo.UDD->type == PHIDTYPE_MESH ? "Mesh" : "Hub", device, packetTracker);
...
phidusb_again:
		phidusbConn = PhidgetPHIDUSBConnectionCast(device->conn);
		assert(phidusbConn);
		res = PhidgetUSBTransferPhidgetPacket(iop, phidusbConn, PHIDGETUSB_REQ_BULK_WRITE, 0, 0, bufferIn, &bufferInLen, 1000);
		switch (res) {
		case EPHIDGET_OK:
		case EPHIDGET_NOTATTACHED:
		case EPHIDGET_INTERRUPTED:
...
transferpacket(mosiop_t iop, PhidgetDeviceHandle device, int transferType, int packetType, int index,
	...
	phidusb_again:
		phidusbConn = PhidgetPHIDUSBConnectionCast(device->conn);
		assert(phidusbConn);
		res = PhidgetUSBTransferPhidgetPacket(iop, phidusbConn, transferType, packetType, index, bufferIn, bufferInLen, timeout);
		switch (res) {
		case EPHIDGET_OK:
		case EPHIDGET_NOTATTACHED:
		case EPHIDGET_INTERRUPTED:
...
PhidgetDevice_transferpacket(mosiop_t iop, PhidgetDeviceHandle device, int transferType, int packetType, int index,
	const unsigned char *bufferIn, size_t *bufferInLen, int timeout) {

	return (transferpacket(iop, device, transferType, packetType, index, bufferIn, bufferInLen, NULL, timeout));
}
