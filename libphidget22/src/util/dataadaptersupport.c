...
// === Internal Functions === //
PhidgetReturnCode processDataPackets(PhidgetChannelHandle ch, const uint8_t *buffer, size_t length) {
	...
	case VINT_PACKET_TYPE_DATAADAPTER_PACKET_DATA:
		...
		if (DATAADAPTER_SUPPORT(ch)->usbInPacketCount != receivedPacketCount || (newResponsePacket && (DATAADAPTER_SUPPORT(ch)->storedPacketLength != 0))) {
			//Send old data to channel
			if (!newResponsePacket) // we missed the start of this packet, it is in error
				error = 1;
			ret = bridgeSendToChannel(ch, BP_DATAIN, "%*R%u%u%uh", DATAADAPTER_SUPPORT(ch)->storedPacketLength, DATAADAPTER_SUPPORT(ch)->storedPacket, error, (DATAADAPTER_SUPPORT(ch)->rxPacketID != ANONYMOUS_PACKET_ID), DATAADAPTER_SUPPORT(ch)->rxPacketID);
			DATAADAPTER_SUPPORT(ch)->storedPacketLength = 0;
		}

		if (DATAADAPTER_SUPPORT(ch)->rxPacketID == NEW_RX_READY) {
			DATAADAPTER_SUPPORT(ch)->rxPacketID = packetID;
			DATAADAPTER_SUPPORT(ch)->rxPacketError = 0;
		}

		DATAADAPTER_SUPPORT(ch)->rxPacketError |= error;

		if (packetID != DATAADAPTER_SUPPORT(ch)->rxPacketID && !packetDropped && !newResponsePacket) {
			MOS_PANIC("TODO: Something got out of sequence");
		}

		//handle the packet
		DATAADAPTER_SUPPORT(ch)->usbInPacketCount = receivedPacketCount + 1;
		DATAADAPTER_SUPPORT(ch)->lastDataLength = length - USB_IN_PACKET_OVERHEAD;
		memcpy(DATAADAPTER_SUPPORT(ch)->lastData, buffer + USB_IN_PACKET_OVERHEAD, DATAADAPTER_SUPPORT(ch)->lastDataLength);

		if (DATAADAPTER_SUPPORT(ch)->storedPacketLength + DATAADAPTER_SUPPORT(ch)->lastDataLength <= DATAADAPTER_MAX_PACKET_LENGTH) {
			memcpy(&DATAADAPTER_SUPPORT(ch)->storedPacket[DATAADAPTER_SUPPORT(ch)->storedPacketLength], DATAADAPTER_SUPPORT(ch)->lastData, DATAADAPTER_SUPPORT(ch)->lastDataLength);
			DATAADAPTER_SUPPORT(ch)->storedPacketLength += (uint16_t)DATAADAPTER_SUPPORT(ch)->lastDataLength;
			if ((endPacket == 0))// && (DATAADAPTER_SUPPORT(ch)->protocol != PROTOCOL_RS422 && DATAADAPTER_SUPPORT(ch)->protocol != PROTOCOL_RS485)) //send async packets immediately, store up syncronous ones
				return EPHIDGET_OK;
		} else
			packetOverrun = 1;

		totalPacketLength = DATAADAPTER_SUPPORT(ch)->storedPacketLength;

		//Send data to channel
		ret = bridgeSendToChannel(ch, BP_DATAIN, "%*R%u%u%uh", DATAADAPTER_SUPPORT(ch)->storedPacketLength, DATAADAPTER_SUPPORT(ch)->storedPacket, DATAADAPTER_SUPPORT(ch)->rxPacketError, packetDropped, packetID);
		...
		PhidgetLock(ch);
		DATAADAPTER_SUPPORT(ch)->completeRXPacketLength = totalPacketLength;
		memcpy(DATAADAPTER_SUPPORT(ch)->completeRXPacket, DATAADAPTER_SUPPORT(ch)->storedPacket, DATAADAPTER_SUPPORT(ch)->completeRXPacketLength);
		DATAADAPTER_SUPPORT(ch)->completeRXPacketID = DATAADAPTER_SUPPORT(ch)->rxPacketID;
		DATAADAPTER_SUPPORT(ch)->rxPacketID = NEW_RX_READY;
		PhidgetBroadcast(ch);
		PhidgetUnlock(ch);

		if (packetOverrun && ret == EPHIDGET_OK) {
			memcpy(&DATAADAPTER_SUPPORT(ch)->storedPacket[DATAADAPTER_SUPPORT(ch)->storedPacketLength], DATAADAPTER_SUPPORT(ch)->lastData, DATAADAPTER_SUPPORT(ch)->lastDataLength);
			DATAADAPTER_SUPPORT(ch)->storedPacketLength += (uint32_t)DATAADAPTER_SUPPORT(ch)->lastDataLength;
		}
	...
	case VINT_PACKET_TYPE_DATAADAPTER_PACKET_TIMEOUT:
		...
		packetID = packetInfo & 0x3FFF;
		DATAADAPTER_SUPPORT(ch)->rxPacketID = NEW_RX_READY;
		ret = bridgeSendToChannel(ch, BP_DATAIN, "%*R%u%u%uh", 0, DATAADAPTER_SUPPORT(ch)->storedPacket, 1, 0, packetID);
		return (EPHIDGET_OK);
	case VINT_PACKET_TYPE_DATAADAPTER_PACKET_DROPPED:
		PhidgetLock(ch);
		DATAADAPTER_SUPPORT(ch)->droppedPacketID = unpack16(&buffer[1]);
		PhidgetBroadcast(ch);
		PhidgetUnlock(ch);
	...
	case VINT_PACKET_TYPE_DATAADAPTER_PACKET_ACK:
		PhidgetLock(ch);
		DATAADAPTER_SUPPORT(ch)->ackID = (unpack16(&buffer[1])) & 0x3FFF;
		PhidgetBroadcast(ch);
		PhidgetUnlock(ch);
		ClearNAK(ch);
		return (EPHIDGET_OK);
	...
	case PHIDCHUID_ADP_RS232_DATAADAPTER_100_USB:
		...
		case PHIDCHCLASS_DATAADAPTER:
			...
			case BP_SETBAUDRATE:
				...
				return PhidgetDevice_transferpacket(bp->iop, device, PHIDGETUSB_REQ_DEVICE_WRITE, VINT_PACKET_TYPE_DATAADAPTER_BAUD_RATE, 0, buffer, &len, 100);
			case BP_SETPARITY:
				len = 1;
	...
	case PHIDCHUID_ADP_RS232_DATAADAPTER_100_VINT:
		...
		case PHIDCHCLASS_DATAADAPTER:
			...
			case BP_SETBAUDRATE:
				len = 4;
				pack32(buffer, getBridgePacketUInt32(bp, 0));
				return sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DATAADAPTER_BAUD_RATE, buffer, len);
			case BP_OPENRESET:
				len = 0;
				dataAdapterSupport->protocol = PROTOCOL_RS232;
...
static PhidgetReturnCode sendTXDataVINT(mosiop_t iop, PhidgetChannelHandle ch, uint8_t *buf, size_t packetLen, PhidgetTransaction *trans){
	...
	if (ret == EPHIDGET_BUSY) {
		...
		ret = waitForNAKClear(iop, ch, 500);
		if (ret != EPHIDGET_OK)
			return ret;
...
PhidgetReturnCode sendDataBuffer(PhidgetChannelHandle ch, size_t len, const uint8_t *buffer, BridgePacket* bp, int waitResposne) {
	...
	if (ch->parent->deviceInfo.class != PHIDCLASS_VINT) {
		if (len > 0) {
			if (len <= (USB_OUT_PACKET_LENGTH - USB_OUT_PACKET_OVERHEAD)) {
				...
				ret = PhidgetDevice_transferpacket(bp->iop, (PhidgetDeviceHandle)ch->parent, PHIDGETUSB_REQ_BULK_WRITE, 0, 0, buf, &packetLen, 5000);
				if (ret != EPHIDGET_OK) {
	...
	for (;;) {
		...
		duration = (mos_gettime_usec() - start) / 1000;
		if (duration >= ((640000 / DATAADAPTER_SUPPORT(ch)->baudRate) + 50)) {
		...
		PhidgetLock(ch);
		if (DATAADAPTER_SUPPORT(ch)->baudRate != 0)
			PhidgetTimedWait(ch, ((640000 / DATAADAPTER_SUPPORT(ch)->baudRate) + 50) - (uint32_t)duration);
		else {
			PhidgetUnlock(ch);
			return (MOS_ERROR(bp->iop, EPHIDGET_NOTCONFIGURED, "Baud Rate Not Configured"));
		}
...
PhidgetDataAdapterSupport_init(PhidgetDataAdapterSupportHandle dataAdapter) {
	...
	dataAdapter->completeRXPacketID = NO_ACTIVE_PACKET;
	dataAdapter->completeRXPacketLength = 0;
