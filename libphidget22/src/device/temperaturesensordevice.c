...
PhidgetTemperatureSensorDevice_dataInput(PhidgetDeviceHandle device, uint8_t *buffer, size_t length) {
	...
	case PHIDUID_1048_1:
		//Temp IC - absolute possible range: -61.1 - 161.16
		ambientTemperature = round_double(unpackfloat(&buffer[16]), 4);
		badConnectionFlags = buffer[20];
		// Don't include the ambient sensor in the loop
		for (i = 0; i < phid->devChannelCnts.numTempInputs - 1; i++) {
			//Thermocouple
			potential[i] = round_double(unpackfloat(&buffer[i*4]), 7);
	...
	for (i = 0; i < phid->devChannelCnts.numTempInputs - 1; i++) {
		...
		if ((badConnectionFlags & (1 << i)) != 0) {
			phid->temperature[i] = PUNK_DBL;
			if (ch != NULL) {
				SEND_ERROR_EVENT(ch, EEPHIDGET_BADCONNECTION,
					"Input reported a bad connection on this channel. Check there is a thermocouple attached.");
			}

		} else if (phid->temperature[phid->ambientSensorIndex] == PUNK_DBL) {
			phid->temperature[i] = PUNK_DBL;
			if (ch != NULL) {
				SEND_ERROR_EVENT(ch, EEPHIDGET_OUTOFRANGE,
		...
		case PHIDUID_1048_1:
			if ((badConnectionFlags & (1 << i)) != 0) {
				...
				if ((ch = getChannel(phid, i + phid->devChannelCnts.numTempInputs)) != NULL) {
					SEND_ERROR_EVENT(ch, EEPHIDGET_BADCONNECTION,
						"Input reported a bad connection on this channel.");
					PhidgetRelease(&ch);
				}

			} else if (potential[i] > phid->potentialMax) {
				phid->voltage[i] = PUNK_DBL;
				if ((ch = getChannel(phid, i + phid->devChannelCnts.numTempInputs)) != NULL) {
					SEND_ERROR_EVENT(ch, EEPHIDGET_OUTOFRANGE,
