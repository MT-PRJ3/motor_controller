...
send1014_3_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_1014_DIGITALOUTPUT_800_VINT:
		...
		case BP_SETDUTYCYCLE:
			buf[0] = (uint8_t)(getBridgePacketDouble(bp, 0) >= 0.5 ? 0xFF : 0x00);
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_DIGITALOUTPUT_SETDUTYCYCLE, buf, 1));
...
send1024_V2_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	...
	case PHIDCHUID_1024_RFID_200_VINT:
		...
		case BP_WRITE:
			...
			strncpy((char*)&buffer[2], tagString, stringLength);
			buffer[2 + stringLength] = '\0';
			if (protocol == PROTOCOL_EM4100) {
				for (i = 0; i < 10; i++) {
					buffer[2+i] = tolower(buffer[2 + i]);
				}
			}
...
static PhidgetReturnCode
sendADP_RS232_VINT(PhidgetChannelHandle ch, BridgePacket *bp) {
	double dutyCycle;
	uint8_t buf[1];
	assert(ch);
	assert(bp);
...
static PhidgetReturnCode
recvADP_RS232_VINT(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	int state;
	int pkt;
...
static PhidgetReturnCode
sendVINTSPATIAL_SETCORRECTIONPARAMETERS(PhidgetChannelHandle ch, BridgePacket *bp) {
	float firmware_userMagnetometerTransform[6];
	float firmware_userMagnetometerOffset[3];
	float firmware_userMagnetometerGain[3];
	float firmware_userMagField;
...
static PhidgetReturnCode
sendVINTSPATIAL(PhidgetChannelHandle ch, BridgePacket *bp) {
	uint8_t buf[4];
	...
	case PHIDCHUID_VINTSPATIAL_SPATIAL_100:
		...
		case BP_SAVECORRECTIONPARAMETERS:
			return (sendVINTDataPacket(bp->iop, ch, VINT_PACKET_TYPE_MAGNETOMETER_SAVECORRECTIONPARAMETERS, NULL, 0));
		case BP_SETCORRECTIONPARAMETERS:
			return (sendVINTSPATIAL_SETCORRECTIONPARAMETERS(ch, bp));
...
static PhidgetReturnCode
recvVINTSPATIAL(PhidgetChannelHandle ch, const uint8_t *buf, size_t len) {
	int fireSaturation = PFALSE;
	PhidgetReturnCode ret;
	double timestamp;
	double quaternion[4];
